...
// Description: Java 11 XML Message SAX Response Record Element  for SecGrpMemb
...
/*
 *	CFBamXMsgRspnSecGrpMembUpdated XML SAX Element  implementation
 *	for SecGrpMemb.
 */
public class CFBamXMsgRspnSecGrpMembUpdated
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnSecGrpMembUpdated( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			assert qName.equals( "RspnSecGrpMembUpdated" );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				else if( attrLocalName.equals( "SecGrpMembId" ) ) {
					if( attrSecGrpMembId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
							attrLocalName );
					}
					attrSecGrpMembId = attrs.getValue( idxAttr );
				}
			...
			if( ( attrSecGrpMembId == null ) || ( attrSecGrpMembId.length() <= 0 ) ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"SecGrpMembId" );
			...
			long natSecGrpMembId = Long.parseLong( attrSecGrpMembId );
			...
			ICFBamSecGrpMembObj obj = (ICFBamSecGrpMembObj)schemaObj.getSecGrpMembTableObj().newInstance();
			CFSecSecGrpMembBuff dataBuff = obj.getSecGrpMembBuff();
			dataBuff.setRequiredClusterId( natClusterId );
			dataBuff.setRequiredSecGrpMembId( natSecGrpMembId );
			...
			obj.copyBuffToPKey();
			ICFBamSecGrpMembObj realised = (ICFBamSecGrpMembObj)obj.realise();
