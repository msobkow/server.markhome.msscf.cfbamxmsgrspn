...
// Description: Java 11 XML Message SAX Response Record Element  for Param
...
/*
 *	CFBamXMsgRspnParamRec XML SAX Element  implementation
 *	for Param.
 */
public class CFBamXMsgRspnParamRec
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnParamRec( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			if( ( attrTypeTenantId == null ) || ( attrTypeTenantId.length() <= 0 ) ) {
				...
				natTypeTenantId = Long.valueOf( Long.parseLong( attrTypeTenantId ) );
			}
			...
			if( ( attrTypeId == null ) || ( attrTypeId.length() <= 0 ) ) {
				...
				natTypeId = Long.valueOf( Long.parseLong( attrTypeId ) );
			}
			...
			List<ICFBamParamObj> list =
				(List<ICFBamParamObj>)xmsgRspn.getListOfObjects();
