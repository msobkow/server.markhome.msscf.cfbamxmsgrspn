...
// Description: Java 11 XML Message SAX Response Record Element  for TableCol
...
/*
 *	CFBamXMsgRspnTableColCreated XML SAX Element  implementation
 *	for TableCol.
 */
public class CFBamXMsgRspnTableColCreated
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnTableColCreated( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			String	attrDataTenantId = null;
			String	attrDataId = null;
			...
			assert qName.equals( "RspnTableColCreated" );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				if( attrLocalName.equals( "Id" ) ) {
				...
				else if( attrLocalName.equals( "DataTenantId" ) ) {
					if( attrDataTenantId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
							attrLocalName );
					}
					attrDataTenantId = attrs.getValue( idxAttr );
				}
				else if( attrLocalName.equals( "DataId" ) ) {
					if( attrDataId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
					...
					attrDataId = attrs.getValue( idxAttr );
				}
			...
			Long natDataTenantId;
			if( ( attrDataTenantId == null ) || ( attrDataTenantId.length() <= 0 ) ) {
				natDataTenantId = null;
			}
			else {
				natDataTenantId = Long.valueOf( Long.parseLong( attrDataTenantId ) );
			}

			Long natDataId;
			if( ( attrDataId == null ) || ( attrDataId.length() <= 0 ) ) {
				natDataId = null;
			}
			else {
				natDataId = Long.valueOf( Long.parseLong( attrDataId ) );
			}
			...
			CFBamTableColBuff dataBuff = obj.getTableColBuff();
			...
			dataBuff.setOptionalDataTenantId( natDataTenantId );
			dataBuff.setOptionalDataId( natDataId );
			dataBuff.setOptionalXmlElementName( natXmlElementName );
			obj.copyBuffToPKey();
