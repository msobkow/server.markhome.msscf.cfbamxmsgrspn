...
// Description: Java 11 XML Message SAX Response Record Element  for Id16Gen
...
/*
 *	CFBamXMsgRspnId16GenRec XML SAX Element  implementation
 *	for Id16Gen.
 */
public class CFBamXMsgRspnId16GenRec
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnId16GenRec( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			// Id16Gen Attributes
			String	attrDispenserTenantId = null;
			String	attrDispenserId = null;
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				else if( attrLocalName.equals( "DispenserTenantId" ) ) {
					if( attrDispenserTenantId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
							attrLocalName );
					}
					attrDispenserTenantId = attrs.getValue( idxAttr );
				}
				else if( attrLocalName.equals( "DispenserId" ) ) {
					if( attrDispenserId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
							attrLocalName );
					}
					attrDispenserId = attrs.getValue( idxAttr );
				}
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= Short.valueOf( Short.parseShort( attrInitValue ) );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= Short.valueOf( Short.parseShort( attrMinValue ) );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= Short.valueOf( Short.parseShort( attrMaxValue ) );
			}

			long natSchemaDefId = Long.parseLong( attrSchemaDefId );

			Long natDispenserTenantId;
			if( ( attrDispenserTenantId == null ) || ( attrDispenserTenantId.length() <= 0 ) ) {
				natDispenserTenantId = null;
			}
			else {
				natDispenserTenantId = Long.valueOf( Long.parseLong( attrDispenserTenantId ) );
			}

			Long natDispenserId;
			if( ( attrDispenserId == null ) || ( attrDispenserId.length() <= 0 ) ) {
				natDispenserId = null;
			}
			else {
				natDispenserId = Long.valueOf( Long.parseLong( attrDispenserId ) );
			}
			...
			CFBamId16GenBuff dataBuff = obj.getId16GenBuff();
			...
			dataBuff.setOptionalDispenserTenantId( natDispenserTenantId );
			dataBuff.setOptionalDispenserId( natDispenserId );
			dataBuff.setRequiredSlice( natSlice );
			dataBuff.setRequiredBlockSize( natBlockSize );
			obj.copyBuffToPKey();
			@SuppressWarnings("unchecked")
			List<ICFBamValueObj> list =
				(List<ICFBamValueObj>)xmsgRspn.getListOfObjects();
