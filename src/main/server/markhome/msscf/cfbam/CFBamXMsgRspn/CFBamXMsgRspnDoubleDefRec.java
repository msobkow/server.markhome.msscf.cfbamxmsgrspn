...
// Description: Java 11 XML Message SAX Response Record Element  for DoubleDef
...
/*
 *	CFBamXMsgRspnDoubleDefRec XML SAX Element  implementation
 *	for DoubleDef.
 */
public class CFBamXMsgRspnDoubleDefRec
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnDoubleDefRec( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= Double.valueOf( Double.parseDouble( attrInitValue ) );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= Double.valueOf( Double.parseDouble( attrMinValue ) );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= Double.valueOf( Double.parseDouble( attrMaxValue ) );
			}

			// Get the parent context
			CFLibXmlCoreContext parentContext = curContext.getPrevContext();
			// Instantiate a buffer for the parsed information
			ICFBamDoubleDefObj obj = (ICFBamDoubleDefObj)(schemaObj.getDoubleDefTableObj().newInstance());
			CFBamDoubleDefBuff dataBuff = obj.getDoubleDefBuff();
