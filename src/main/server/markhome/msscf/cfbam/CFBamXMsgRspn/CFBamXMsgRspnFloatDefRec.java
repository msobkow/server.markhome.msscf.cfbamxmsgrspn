...
// Description: Java 11 XML Message SAX Response Record Element  for FloatDef
...
/*
 *	CFBamXMsgRspnFloatDefRec XML SAX Element  implementation
 *	for FloatDef.
 */
public class CFBamXMsgRspnFloatDefRec
	extends CFLibXmlCoreElementHandler
{
	public CFBamXMsgRspnFloatDefRec( CFBamXMsgRspn xmsgRspn ) {
		super( xmsgRspn );
	}
	...
	throws SAXException
	{
		try {
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= Float.valueOf( Float.parseFloat( attrInitValue ) );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= Float.valueOf( Float.parseFloat( attrMinValue ) );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= Float.valueOf( Float.parseFloat( attrMaxValue ) );
			}

			// Get the parent context
			CFLibXmlCoreContext parentContext = curContext.getPrevContext();
			// Instantiate a buffer for the parsed information
			ICFBamFloatDefObj obj = (ICFBamFloatDefObj)(schemaObj.getFloatDefTableObj().newInstance());
			CFBamFloatDefBuff dataBuff = obj.getFloatDefBuff();
